...
namespace MonsterWorldApi.Controllers.v2
{
    ...
    public class MonstersController : CommunicationsController
    {
        ...
        /// <returns></returns>
        [HttpGet]//Get por dificult
        [Route("fight/{dificulty}")]//Adicionada a rota /fight/dificulty pq apesar de nao testar pensei que fosse ter conflito entre o get por id
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [Produces(MediaTypeNames.Application.Json)]
        public IActionResult GetFight(Dificulties dificulty)//parecido com o get por id, porém temos que passar o numero relativo a dificuldade do monstro que vc deseja de forma aleatoria
        {
            _logger.LogInformation($"O Usuário está gerando um combate de monstros{nameof(Get)} as {DateTime.Now}");

            var exists = _service.GetFight(dificulty); //var exists recebe valor do metodo get com dificuldade como parametro
            if (exists != null)
            {
                _logger.LogInformation($"O Usuário  obteve o resultado do duelo. {nameof(Get)} as {DateTime.Now}");
                return ApiOk(exists);
            } // se exists for diferente de null retorna ok, se nao retorna o erro
            else
            {
                _logger.LogError($"O Usuário não conseguiu obter um duelo  {nameof(Get)} as {DateTime.Now}");

                return ApiNotFound<Monster>("Monstros não existem");
            }
